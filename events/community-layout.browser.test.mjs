...
test('0.6.3 version navigation and event cards stay within desktop and portrait widths',async()=>{
 ...
 await page.setContent(`<style>body{margin:0}.wrapper{display:grid;grid-template-columns:repeat(2,1fr);width:1150px}.community-track-versions{height:488px}.hidden{display:none}</style><div id="ui"><div class="track-selection-ui"><div class="tracks-container"><div class="wrapper"><section class="sq-featured-events sq-event-track-group"><h2 class="sq-featured-heading">Featured events</h2><div class="track track-of-the-week sq-kodub-weekly"><button class="button"><small class="sq-kodub-label">Kodub weekly</small><img class="cover"><div class="track-of-the-week-info"><div class="title">going back in time</div><div class="time-remaining">17h remaining</div></div></button></div><button class="sq-events-entry">Events</button><div class="sq-event-track-buttons"><button class="sq-event-card">Weekly event</button><button class="sq-event-card">Daily event</button></div></section><div class="community-track-versions"><button class="button">StaticQuasar931</button><button class="button">0.6.3</button><button class="button">0.6.1</button></div><div class="community-track-group"><div class="track"><button class="button">Rolling Hills Racer</button></div></div></div></div></div></div>`);
 ...
 for(const [width,height] of [[1366,768],[390,844],[768,1024]]){
  ...
  for(const selector of ['.sq-event-track-group','.sq-event-card','.sq-kodub-weekly','.sq-kodub-weekly .title','.sq-kodub-weekly .time-remaining'])for(const item of await page.locator(selector).all()){
