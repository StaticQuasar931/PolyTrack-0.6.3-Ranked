...
test('0.6.3 version navigation and event cards stay within desktop and portrait widths',async()=>{
 ...
 for(const [width,height] of [[1366,768],[390,844],[768,1024]]){
  ...
  assert.equal(rewardBoxes.length,3);for(const box of rewardBoxes)assert(Math.abs(box.y-rewardBoxes[0].y)<=1,"Record rows align within subpixel rounding");
 ...
 assert.equal((await page.locator('.sq-kodub-weekly>button').boundingBox()).height,260);
 assert.equal(image.fit,'contain');assert.equal(image.height,260);
